 ...
 * la fila del staging): la placa no existe en Vehículos, o el Tipo de Ruta
 * no tiene Centro de Costo asignado en el catálogo. `validar.ts` arma el
 * mensaje con el builder; la UI del importador usa el extractor
 * correspondiente para reconocer esos mensajes y mostrar un enlace directo
 * a corregirlos, en vez de comparar strings a mano en dos lugares (lo que
 * se desincroniza fácil).
 */
